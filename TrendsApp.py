...
def process_data(df):
    ...
    meta_columns = [
        x for x in st.session_state['df'].columns.tolist() if x not in ["OTU", "level_1", "RA", "Notes"]
    ]
    
    st.session_state['df'][meta_columns] = st.session_state['df'][meta_columns].astype(str)
